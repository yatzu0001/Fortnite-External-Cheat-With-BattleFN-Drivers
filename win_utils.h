...
Type read(void* DriverHandle, unsigned long int Process_Identifier, unsigned long long int Address)
{
	SIZE_T REAAD;
	Type returnValue;
	ReadProcessMemory(processHandle, (LPCVOID)Address, &returnValue, sizeof(Type), &REAAD);
...
{
	SIZE_T written;
	WriteProcessMemory(processHandle, (LPVOID)Address, &stuff, sizeof(float), &written); // changed here yatzu#7293 
}
...
{
	SIZE_T written;
	WriteProcessMemory(processHandle, (LPVOID)Address, &stuff, sizeof(double), &written); // changed here yatzu#7293 
}
